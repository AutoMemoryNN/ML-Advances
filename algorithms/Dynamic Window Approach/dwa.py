...
for i in range(25):
    (bx, by, vx, vy) = (
        ...
        random.gauss(0.0, BARRIERVELOCITYRANGE), random.gauss(0.0, BARRIERVELOCITYRANGE))
    barrier = [bx, by, vx, vy]
...
def drawBarriers(barriers):
    for (i, barrier) in enumerate(barriers):
        if (i == targetindex):
            ...
            bcol = lightblue
        pygame.draw.circle(screen, bcol, (int(u0 + k * barrier[0]), int(v0 - k * barrier[1])), int(k * BARRIERRADIUS),
                           0)
...
while (1):
    ...
    for vLpossible in vLpossiblearray:
        for vRpossible in vRpossiblearray:
            ...
            if (
                    vLpossible <= MAXVELOCITY and vRpossible <= MAXVELOCITY and vLpossible >= -MAXVELOCITY and vRpossible >= -MAXVELOCITY):
